...
class ASCIICraft {
  ...
  demoCube: THREE.Mesh | null;
  mediaPlane: THREE.Mesh | null;
  canvasRatio: number;
  ...
  setupMediaScene(texture: THREE.Texture) {
    this.clearScene();

    this.mediaTexture = texture;
    const image = texture.image as HTMLVideoElement | HTMLImageElement;
    const mediaWidth = (image as HTMLVideoElement).videoWidth || (image as HTMLImageElement).width;
    const mediaHeight = (image as HTMLVideoElement).videoHeight || (image as HTMLImageElement).height;
  ...
  updateMediaPlane() {
    ...
    const image = this.mediaTexture.image as HTMLVideoElement | HTMLImageElement;
    const mediaWidth = (image as HTMLVideoElement).videoWidth || (image as HTMLImageElement).width;
    const mediaHeight = (image as HTMLVideoElement).videoHeight || (image as HTMLImageElement).height;
    ...
    if (this.mediaPlane) {
      ...
      if (Array.isArray(this.mediaPlane.material)) {
        for (const m of this.mediaPlane.material) {
          m.dispose();
        }
      } else {
  ...
  loadImage(file: File): Promise<THREE.Texture> {
  ...
  loadVideo(file: File): Promise<THREE.Texture> {
  ...
  async startWebcam(): Promise<THREE.Texture> {
  ...
  async setSource(source: string, file: File | null = null) {
    ...
    switch (source) {
      ...
      case "webcam":
        ...
        try {
          ...
          (document.getElementById("source-select")! as HTMLSelectElement).value = "demo";
          this.setSource("demo");
        }
        break;
    }
  ...
  setupControls() {
    ...
    sourceSelect.addEventListener("change", (e) => {
      this.setSource((e.target as HTMLSelectElement).value);
    });
    ...
    fileInput.addEventListener("change", async (e) => {
      ...
      (sourceSelect as HTMLSelectElement).value = newSource;
      await this.setSource(newSource, file);
    });

    targetFpsInput.addEventListener("input", (e) => {
      const value = parseInt((e.target as HTMLInputElement).value, 10);
      this.targetFPS = value;
      document.getElementById("target-fps-value")!.textContent = value.toString();
    });

    cellSizeInput.addEventListener("input", (e) => {
      const value = parseInt((e.target as HTMLInputElement).value, 10);
      this.asciiEffect.cellSize = value;
      document.getElementById("cell-size-value")!.textContent = value.toString();
    });

    canvasRatioSelect.addEventListener("change", (e) => {
      this.canvasRatio = parseInt((e.target as HTMLSelectElement).value, 10);
      this.applyCanvasRatio();
    });

    fontFamilySelect.addEventListener("change", (e) => {
      this.asciiEffect.fontFamily = (e.target as HTMLSelectElement).value;
    });

    bgCharInput.addEventListener("change", () => {
      this.asciiEffect.setCharacters(
        (bgCharInput as HTMLInputElement).value,
        (patternCharsInput as HTMLInputElement).value,
      );
    });
    ...
    colorDarkInput.addEventListener("input", (e) => {
      this.asciiEffect.colorDark = (e.target as HTMLInputElement).value;
    });

    colorLightInput.addEventListener("input", (e) => {
      this.asciiEffect.colorLight = (e.target as HTMLInputElement).value;
    });
    ...
    bgColorInput.addEventListener("input", (e) => {
      this.asciiEffect.bgColor = (e.target as HTMLInputElement).value;
    });

    invertInput.addEventListener("change", (e) => {
      this.asciiEffect.invert = (e.target as HTMLInputElement).checked;
    });

    colorModeInput.addEventListener("change", (e) => {
      this.asciiEffect.colorMode = (e.target as HTMLInputElement).checked;
    });
  }
  ...
  setupRecordingControls() {
    ...
    recordFpsInput.addEventListener("input", (e) => {
      recordFps = parseInt((e.target as HTMLInputElement).value, 10);
      document.getElementById("record-fps-value")!.textContent = recordFps.toString();
      updateDurationPreview();
    });

    recordMaxFramesInput.addEventListener("input", (e) => {
      maxFrames = parseInt((e.target as HTMLInputElement).value, 10);
      document.getElementById("record-max-frames-value")!.textContent = maxFrames.toString();
      updateDurationPreview();
    });

    if (lumBitsSelect) {
      lumBitsSelect.addEventListener("change", (e) => {
        lumBits = parseInt((e.target as HTMLSelectElement).value, 10);
      });
    }
    ...
    recordBtn.addEventListener("click", () => {
      if (this.recorder?.isRecording) {
        ...
        recordMaxDisplay.textContent = maxFrames.toString();

        this.recorder = new ASCIIRecorder({
          renderer: this.renderer,
          asciiEffect: this.asciiEffect as unknown as ASCIIRecorderOptions["asciiEffect"],
          scene: this.scene,
          camera: this.camera,
          fps: recordFps,
          maxFrames: maxFrames,
          ...
          onFrame: ({ frame, elapsed }) => {
            ...
            recordFrames.textContent = frame.toString();
          },
          onComplete: () => {
            const stats = this.recorder?.downloadTXA(`ascii-${Date.now()}.txa`);
            recordBtn.textContent = "Start Recording";
  ...
  setupDragDrop() {
    ...
    document.addEventListener("drop", async (e) => {
      ...
      (document.getElementById("source-select")! as HTMLSelectElement).value = newSource;
      await this.setSource(newSource, file);
    });
  }
